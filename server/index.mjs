...
app.post("/add-post", authenticate, (req, res) => {
  const {
    title,
    subtitle,
    imageUrl,
...
app.get("/posts", (req, res) => {
  fs.readdir(blogDir, (err, files) => {
    ...
    const posts = files.map((file) => {
      ...
      const post = {};
      lines.forEach((line) => {
        if (line.startsWith("ocultar:"))
          post.ocultar = line.replace("ocultar: ", "").replace(/"/g, "");
        if (line.startsWith("titulo:"))
          post.titulo = line.replace("titulo: ", "").replace(/"/g, "");
        if (line.startsWith("subtitulo:"))
          post.subtitulo = line.replace("subtitulo: ", "").replace(/"/g, "");
        /* if (line.startsWith("imagen:"))
          post.imagen = JSON.parse(line.replace("imagen: ", "")); */
        if (line.startsWith("fecha:"))
          post.fecha = line.replace("fecha: ", "").replace(/"/g, "");
        if (line.startsWith("categoria:"))
          post.categoria = line.replace("categoria: ", "").replace(/"/g, "");
        if (line.startsWith("autor:"))
          post.autor = line.replace("autor: ", "").replace(/"/g, "");
